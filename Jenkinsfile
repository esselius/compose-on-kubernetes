                ...
                stage('Test e2e kube 1.11') {
                    ...
                    steps {
                        ...
                        dir('src/github.com/docker/compose-on-kubernetes') {
                            ...
                            sh 'docker run --rm -e CLUSTER_NAME=$CLUSTERNAME -e USER_ID=$(id -u) -e GROUP_ID=$(id -g) -v $HOME/.kube:/storage -v /var/run/docker.sock:/var/run/docker.sock kind-helper'
                            sh 'ls -lha $HOME/.kube'
                            script {
                                env.KUBECONFIGFILE = sh(
                                    script: 'docker run kind-helper kind get kubeconfig-path --name=$CLUSTER_NAME',
                                    returnStdout: true,
                                )
                                env.KUBECONFIG = sh(
                                    script: 'echo -n $HOME/.kube/${KUBECONFIG##*/}',
                                    returnStdout: true,
                                )
                            }
