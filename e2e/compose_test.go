...
services:
  back:
    image: nginx:1.12.1-alpine`
		...
		err = ns.RESTClientV1beta2().Get().Namespace(ns.Name()).Name("app").Resource("stacks").SubResource("composefile").Do().Into(&cf)
		expectNoError(err)
...
services:
  test:
    ...
    - /bin/sh
    - -c
    - "grep http /tmp/hostetc/services ; sleep 3600"
    volumes:
      - type: bind
        source: /etc
        target: /tmp/hostetc`)
		...
		expectNoError(err)
		sdata := string(data)
		Expect(strings.Contains(sdata, "443/tcp")).To(BeTrue())
	})
